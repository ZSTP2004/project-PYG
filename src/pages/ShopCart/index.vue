<template>
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body" >
        <ul class="cart-list" v-for="(cartInfoList, index) in cartList.cartInfoList" :key="cartInfoList.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked= "cartInfoList.isChecked == 1" @change="updateChecked(cartInfoList,$event)">
          </li>
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="cartInfoList.skuNum" minnum="1" class="itxt" @change="handler('change',$event.target.value * 1,cartInfoList )">
          ...
          <li class="cart-list-con7">
            <a href="#none" class="sindelet" @click="deleteCartById(cartInfoList)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
import { mapGetters } from 'vuex'
  ...
  export default {
    ...
    methods:{
      ...
      async handler(type,disNum,cart){
        ...
        //  -- 
          ...
          case 'change':    //此时 disNum 是实际的值
            // 判断用户输入进来的是否非法 或负数
            if(isNaN(disNum) || disNum < 1){
              disNum = 0
            }else{ // 正常情况  除去小数  （输入后的输 - 之前的数    为正就+   负就-）
    ...
    computed:{
      ...
      // 判断底部复选框是否勾选
      isAllCheck(){
        return this.cartList.cartInfoList.every(item => item.isChecked == 1)
      }
    }
  }
</script>
