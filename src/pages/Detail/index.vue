    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea"  v-for="(spuSale, index) in spuSaleAttrList" :key="spuSale.id">
              <div class="choosed"></div>
              <dl>
                <dt class="title" >{{ spuSale.saleAttrName }}</dt>
                <dd changepirce="0" :class="{active:spuSaleAttrValue.isChecked == 1}"  
                v-for="(spuSaleAttrValue, index) in spuSale.spuSaleAttrValueList" :key="spuSaleAttrValue.id" @click="changeActive(spuSaleAttrValue,spuSale.spuSaleAttrValueList)">
...
<script>
  ...
  export default {
    ...
    methods:{    // 点击的属性      全部
      changeActive(spuSaleAttrValue,spuSaleArr){
        // 排他思想  先遍历全部售卖属性值 让他们都不亮
        spuSaleArr.forEach(item => {
          item.isChecked = 0
        });
        // 点击的那个售卖属性高亮
      ...
      // 表单元素修改产品个数
      changeSkuNum(event){
        // 输入的文本 * 1   排除非法字符...   isNaN
        let value =  event.target.value * 1
        if(isNaN(value) || value < 1){
          this.skuNum = 1
        }else{
          // 正常大于1 【大于 1 整数 不能出现小数】
          this.skuNum =  parseInt(value)
        }
       
      },
      // 添加至购物车
      async addShopCart(){      //10.29
        // 点击完后发请求  将产品的相关参数带个服务器 
         try {
